      ...
      >
        {[
          ...
          <motion.div 
            ...
            transition={{ type: "spring", stiffness: 400, damping: 30 }}
            className={`p-10 rounded-[2rem] border shadow-inner flex flex-col justify-between min-h-[250px] overflow-hidden relative group backdrop-blur-md ${service.span} ${service.highlight ? `bg-gradient-to-br ${theme.from600} ${theme.to800} ${theme.border500_30}` : 'bg-slate-900/50 border-white/5 hover:bg-slate-900/80'}`}
          >
            {/* Background Glow */}
            <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${service.highlight ? 'bg-white/10' : `bg-gradient-to-br ${theme.from500_10} to-transparent`}`} />
            
            <div className={`w-16 h-16 rounded-3xl flex items-center justify-center mb-8 relative z-10 border ${service.highlight ? 'bg-white/20 border-white/30 text-white' : `bg-white/5 border-white/10 ${theme.text400} ${theme.groupHoverBorder500_30}`} transition-colors duration-300`}>
              {service.icon}
            </div>
            <div className="relative z-10">
              <h3 className={`text-3xl font-bold mb-4 tracking-tight ${service.highlight ? 'text-white' : 'text-white'}`}>{service.title}</h3>
              <p className={`mb-8 text-xl font-light ${service.highlight ? theme.text50 : 'text-slate-400'}`}>{service.desc}</p>
              <ul className="space-y-4">
                {[1, 2].map((_, j) => (
                  <li key={j} className="flex items-center gap-4 opacity-90">
                    <div className={`flex items-center justify-center w-6 h-6 rounded-full ${service.highlight ? 'bg-white/20' : `${theme.bg500_20} ${theme.text400}`}`}>
